<template>
  <section class="flex flex-col gap-2 sm:gap-3">
    <h2 class="text-[10px] sm:text-[11px] font-bold tracking-widest uppercase text-[#9ecfa8]">INTELLECTUAL PROPERTY</h2>
    ...
    <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-2.5">
      ...
      >
        ...
        <span class="text-[9px] sm:text-[11px] mt-0.5" :class="{ 'text-[#e6a817]': s.subClass === 'warn', 'text-[#2ecc71]': s.subClass === 'ok', 'text-white/40': !s.subClass }">{{ s.sub }}</span>
      </div>
    </section>
    ...
    <section class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-2.5">
      <div class="rounded-xl px-3 sm:px-4 py-2.5 sm:py-3.5 flex flex-col gap-1 min-h-[75px] sm:min-h-[90px] bg-[#263e30] border border-white/10 shadow-[3px_3px_6px_rgba(0,0,0,0.5)]">
        <p class="text-[10px] sm:text-xs font-semibold text-white mb-2 sm:mb-3.5">IP by classification</p>
        <div class="flex flex-col gap-2 sm:gap-3">
          <div v-for="item in ipByClassification" :key="item.label" class="flex items-center gap-1.5 sm:gap-2.5">
            <span class="text-[9px] sm:text-xs text-white/60 w-[85px] sm:w-[130px] shrink-0">{{ item.label }}</span>
      ...
      <div class="rounded-xl px-3 sm:px-4 py-2.5 sm:py-3.5 flex flex-col gap-1 min-h-[75px] sm:min-h-[90px] bg-[#263e30] border border-white/10 shadow-[3px_3px_6px_rgba(0,0,0,0.5)]">
        <p class="text-[10px] sm:text-xs font-semibold text-white mb-2 sm:mb-3.5">IP by status</p>
        <div class="flex flex-col gap-2 sm:gap-3">
          <div v-for="item in ipByStatus" :key="item.label" class="flex items-center gap-1.5 sm:gap-2.5">
            ...
            <div class="flex-1 h-1 sm:h-1.5 bg-white/10 rounded-full overflow-hidden">
              <div class="h-full rounded-full transition-all duration-300" :style="{ width: barPercent(item.value, maxStatus) + '%', background: item.color }"></div>
            </div>
