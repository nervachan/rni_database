  ...
  <div class="min-h-screen bg-gray-100 font-sans">
    ...
    <div class="px-4 py-4 flex items-center justify-between gap-4 bg-gray-100">
      <div class="flex items-center gap-2 bg-[#d9d9d9] rounded-full px-4 py-2 w-80">
        ...
        <input v-model="search" type="text" class="bg-transparent outline-none text-sm text-gray-600 w-full" aria-label="Search IP records" />
      </div>
      <button
          ...
          >
            ...
            <td class="py-3 px-3">
              <div class="flex items-center gap-1.5">
                <button class="w-7 h-7 flex items-center justify-center rounded border border-[#7b68ee] text-[#7b68ee] hover:bg-[#7b68ee] hover:text-white transition-colors" title="View" @click="$emit('view', row)">
                  <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>
                  </svg>
                </button>
                <button class="w-7 h-7 flex items-center justify-center rounded border border-[#e6a817] text-[#e6a817] hover:bg-[#e6a817] hover:text-white transition-colors" title="Edit" @click="$emit('edit', row)">
                  <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                    <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
                  </svg>
                </button>
                <button class="w-7 h-7 flex items-center justify-center rounded border border-[#e05c5c] text-[#e05c5c] hover:bg-[#e05c5c] hover:text-white transition-colors" title="Delete" @click="$emit('delete', row.id)">
                  <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
