...
function handleSignUp() {
    ...
    if (!success) {
        registrationError.value = auth.error;
        return;
    }
...
<template>
  <div class="RegistrationPage bg-gradient-to-b from-[#203429] to-[#ffffff] flex items-center justify-center h-screen">
    <div class="RegistrationCard bg-gradient-to-b from-[#9abba4] to-[#ffffff] flex flex-col items-center justify-start rounded-lg shadow-xl p-5 sm:p-8 w-full max-w-[400px] mx-4 relative">
      <button
        class="BackButton absolute top-3 left-3 w-7 h-8 bg-[#2e4e3c] hover:opacity-80 rounded-sm flex items-center justify-center"
        @click="goBack"
      >
      ...
      <div class="CardContent flex flex-col items-center gap-4">
        ...
        <div class="BottomRow flex flex-row gap-2 w-full">
          <select
            ...
            class="flex-1 bg-gray-100 drop-shadow-md rounded-md p-2 w-full hover:outline-none hover:ring-2 hover:ring-[#263e30] focus:outline-none focus:ring-2 focus:ring-[#263e30]"
          >
          ...
          <button
            @click="handleSignUp"
            class="fixed-width bg-[#2e4e3c] text-white rounded-md p-2 hover:outline-none hover:opacity-80"
          >
            Sign Up
          </button>
        </div>
