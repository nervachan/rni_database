      ...
      <div class="rounded-xl px-3 sm:px-4 py-2.5 sm:py-3.5 flex flex-col gap-1 min-h-[75px] sm:min-h-[90px] bg-white border border-white/10 shadow-[-3px_3px_6px_rgba(0,0,0,0.25)]">
        ...
        <div class="flex flex-col gap-2 sm:gap-3">
          <div v-for="item in ipByStatus" :key="item.label" class="flex items-center gap-1.5 sm:gap-2.5">
            ...
            <span class="w-[18px] sm:w-[22px] h-[18px] sm:h-[22px] rounded-full flex items-center justify-center text-[9px] sm:text-[11px] font-bold text-grey shrink-0" :style="{ background: item.color }">{{ item.value }}</span>
          </div>
        </div>
      </div>
    </div>
