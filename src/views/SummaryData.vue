<template>
<div class="min-h-screen bg-[#b5c4b1] flex flex-col gap-4 sm:gap-7 font-sans" :class="$route.path === '/dashboard' ? 'p-3 sm:p-5' : 'px-3 py-3 sm:px-4 sm:py-5'"> <!-- frame (2 slots) -->
      <!-- STARTUPS SECTION -->
    <section class="flex flex-col gap-2 sm:gap-3">
      ...
      <!-- Stat cards (container slot 1)-->
      <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-2.5 drop-shadow-md p-2.5 sm:p-4 rounded-xl"> <!-- nested frame (1 row, 2 slots) -->
        <div 
          v-for="s in startupStats" :key="s.eyebrow"
          class="rounded-xl px-3 sm:px-4 py-2.5 sm:py-3.5 flex flex-col gap-1 min-h-[75px] sm:min-h-[90px] shadow-black-950 shadow-[3px_3px_6px_rgba(0,0,0,0.8)] sm:shadow-[5px_5px_10px_rgba(0,0,0,1)] bg-gradient-to-b from-[#1C5333] to-[#0B2C19] border border-[#0B2C19]"> <!-- row -->
          <span class="text-[10px] sm:text-[11px] font-medium text-[#9ecfa8] ">{{ s.eyebrow }}</span>
          <span
            class="font-bold text-white leading-tight"
            :class="s.large ? 'text-[18px] sm:text-[22px]' : 'text-[20px] sm:text-[26px]'"
          >{{ s.value }}</span>
      ...
      <section class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-2.5">
        <div class="rounded-xl px-3 sm:px-4 py-2.5 sm:py-3.5 flex flex-col gap-1 min-h-[75px] sm:min-h-[90px] shadow-black-950 shadow-[3px_3px_6px_rgba(0,0,0,0.5)] sm:shadow-[5px_5px_10px_rgba(0,0,0,0.5)] bg-gradient-to-b from-[#1C5333] to-[#0B2C19] border border-[#2a3a28]">
    ...
    <section class="flex flex-col gap-2 sm:gap-3">
      ...
      <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-2.5">
        <div
          ...
          class="rounded-xl px-3 sm:px-4 py-2.5 sm:py-3.5 flex flex-col gap-1 min-h-[75px] sm:min-h-[90px] shadow-black-950 shadow-[3px_3px_6px_rgba(0,0,0,0.5)] sm:shadow-[5px_5px_10px_rgba(0,0,0,0.5)] bg-gradient-to-b from-[#1C5333] to-[#0B2C19] border border-[#2a3a28]"
        >
          <span class="text-[10px] sm:text-[11px] font-medium text-[#9ecfa8]">{{ s.eyebrow }}</span>
          <span class="text-[20px] sm:text-[26px] font-bold text-white leading-tight">{{ s.value }}</span>
          <span
            class="text-[9px] sm:text-[11px] mt-0.5"
            :class="{
        ...
        <div class="rounded-xl px-3 sm:px-4 py-2.5 sm:py-3.5 flex flex-col gap-1 min-h-[75px] sm:min-h-[90px] shadow-black-950 shadow-[3px_3px_6px_rgba(0,0,0,0.5)] sm:shadow-[5px_5px_10px_rgba(0,0,0,0.5)] bg-gradient-to-b from-[#1C5333] to-[#0B2C19] border border-[#2a3a28]">
          ...
          <div class="flex flex-col gap-2 sm:gap-3">
            <div v-for="item in ipByStatus" :key="item.label" class="flex items-center gap-1.5 sm:gap-2.5">
              ...
              <span
                class="w-[18px] sm:w-[22px] h-[18px] sm:h-[22px] rounded-full flex items-center justify-center text-[9px] sm:text-[11px] font-bold text-white shrink-0"
